...
public class CryptoConfig {
    ...
    private void validateConfiguration() {
        ...
        if (baseSecret.length() < 32) {
            log.warn("⚠️ 基础密钥长度不足32字符，建议使用更强的密钥");
        }

        // 检查时间窗口设置
        if (timeWindow > 600000) { // 超过10分钟
            log.warn("⚠️ 时间窗口设置过长({}ms)，可能存在安全风险", timeWindow);
        }

        if (timeWindow < 60000) { // 少于1分钟
            log.warn("⚠️ 时间窗口设置过短({}ms)，可能影响用户体验", timeWindow);
        }

        // 检查AES密钥大小
        if (aesKeySize < 256) {
            log.warn("⚠️ AES密钥长度不足256位，建议使用AES-256");
        }
